...
import {Avatar, Card, CardContent, CardHeader, IconButton, InputBase, List} from "@mui/material";
...
export const ChatModal: FC<PropsType> = ({onCloseButtonClick, messages}) => {
    ...
    const onMessageTextChange = (e: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
        setMessage(e.currentTarget.value)
    }
    const onKeyUpEnter = (e: KeyboardEvent<HTMLTextAreaElement | HTMLInputElement>) => {
        if (e.key === 'Enter') {
            onSendMessageClick()
        }
    }
    ...
    return (
        <Card sx={{position: 'absolute', bottom: 0, right: '30px', width: 400, height: 500}}>
            ...
            <CardContent sx={{display: 'flex', flexDirection: 'column', height: 400}}>
                ...
                <InputBase
                    style={{width: '100%', padding: '5px 10px 5px 18px', background: '#efefef', flex: '1 0 auto'}}
